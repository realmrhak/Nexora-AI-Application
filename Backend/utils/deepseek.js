...
// ✅ FIXED: Only working models, no dead ones
...
export const generateFlashcards = async (text, count = 10) => {
  ...
  const randomStart = Math.floor(Math.random() * Math.max(1, words.length - 12000));
  const randomText = words.slice(randomStart, randomStart + 12000).join(" ");

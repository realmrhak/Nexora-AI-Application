...
const QuizTakePage = () => {
  ...
  return (
    <div className='w-full max-w-4xl mx-auto px-3 sm:px-4 lg:px-6 py-4 sm:py-6'>
      ...
      {/* Question Card */}
      <div className="bg-white/80 backdrop-blur-xl border-2 border-slate-200 rounded-xl sm:rounded-2xl shadow-xl shadow-slate-200/50 p-4 sm:p-6 mb-6 sm:mb-8">
        <div className="inline-flex items-center gap-2 px-3 sm:px-4 py-1.5 sm:py-2 bg-linear-to-r from-emerald-50 to-teal-50 border border-emerald-200 rounded-lg sm:rounded-xl mb-4 sm:mb-6">
          <div className="w-1.5 h-1.5 sm:w-2 sm:h-2 bg-emerald-500 rounded-full animate-pulse" />
          <span className="text-xs sm:text-sm font-semibold text-emerald-700">
            Question {currentQuestionIndex + 1}
          </span>
        </div>
        <h3 className="text-base sm:text-lg font-semibold text-slate-900 mb-4 sm:mb-6 leading-relaxed">
        ...
        <div className="space-y-2 sm:space-y-3">
          {currentQuestion.options.map((option, index) => {
            ...
            return (
              <label
                key={index} className={`group relative flex items-center p-2.5 sm:p-3 border-2 rounded-lg sm:rounded-xl cursor-pointer transition-all duration-200 ${isSelected
                ...
                {/* Custom Radio Button */}
                <div className={`shrink-0 w-4 h-4 sm:w-5 sm:h-5 rounded-full border-2 transition-all duration-200 ${isSelected
                ...
                {/* Option Text */}
                <span className={`ml-3 sm:ml-4 text-sm sm:text-base font-medium transition-colors duration-200 ${isSelected ? 'text-emerald-900' : 'text-slate-700 group-hover:text-slate-900'
                  }`}>
                  {option}
                </span>
                ...
                {/* Selected Checkmark */}
                {isSelected && (
                  <CheckCircle2 className='ml-auto w-4 h-4 sm:w-5 sm:h-5 text-emerald-500' strokeWidth={2.5} />
                )}
              </label>
            );
          })}
      ...
      <div className="mt-4 sm:mt-6 flex items-center justify-center gap-1.5 sm:gap-2 flex-wrap">
        {quiz.questions.map((_, index) => {
          ...
          return (
            <button
              ...
              disabled={submitting}
              className={`w-7 h-7 sm:w-8 sm:h-8 rounded-md sm:rounded-lg font-semibold text-xs transition-all duration-200 ${isCurrent
